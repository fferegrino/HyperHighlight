  ...
  let toggleCommand = vscode.commands.registerCommand(
    ...
    async () => {
      ...
      vscode.window.showInformationMessage(
        `HyperHighlight: ${hyperHighlightEnabled ? "Enabled" : "Disabled"}`,
      );

      statusBar.text = `HyperHighlight: ${hyperHighlightEnabled ? "Enabled" : "Disabled"}`;
      if (hyperHighlightEnabled) {
        disposable = vscode.window.onDidChangeTextEditorSelection(
